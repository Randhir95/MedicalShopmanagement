...
public class VIewCustomers extends JInternalFrame{
    ...
    public VIewCustomers()
    {
        ...
        String Heding[]={"SR","CUSTOMER NAME","MOBILE NO..","ADDRESS","EMAIL ID","DL NO.","GSTIN NO.","TIN NO","BANK NAME"};
        
        this.addInternalFrameListener(new InternalFrameAdapter(){
             public void internalFrameClosing(InternalFrameEvent e) {
                StaticMember.VIEW_CUSTOMER_DETAILS=false;
              }});
        ...
        customer.setModel(tableModel);//set table model in table
        customer.getTableHeader().setResizingAllowed(false);//stop resizing cell
        customer.getTableHeader().setReorderingAllowed(false);//stop moving cell order
        customer.getTableHeader().setFont(StaticMember.headfont);
        customer.getTableHeader().setBackground(StaticMember.THBcolor);
        customer.getTableHeader().setForeground(StaticMember.THFcolor);
        customer.setFont(StaticMember.itemfont);
        customer.setBackground(StaticMember.TIBcolor);
        customer.setForeground(StaticMember.TIFcolor);
        customer.setRowHeight(20);
        TableColumn column=null;
        column=customer.getColumnModel().getColumn(0);
        column.setPreferredWidth(5);
        column=customer.getColumnModel().getColumn(1);
        column.setPreferredWidth(130);
        column=customer.getColumnModel().getColumn(2);
        column.setPreferredWidth(50);
        column=customer.getColumnModel().getColumn(4);
        column.setPreferredWidth(120);
        column=customer.getColumnModel().getColumn(5);
        column.setPreferredWidth(70);
        column=customer.getColumnModel().getColumn(6);
        column.setPreferredWidth(80);
        column=customer.getColumnModel().getColumn(7);
        column.setPreferredWidth(70);
        column=customer.getColumnModel().getColumn(8);
        column.setPreferredWidth(110);
        DefaultTableCellRenderer rightRenderer=new DefaultTableCellRenderer();
        rightRenderer.setHorizontalAlignment(JLabel.RIGHT);
        DefaultTableCellRenderer centerRenderer=new DefaultTableCellRenderer();
        centerRenderer.setHorizontalAlignment(JLabel.CENTER);
        customer.getColumnModel().getColumn(0).setCellRenderer(centerRenderer);
        customer.getColumnModel().getColumn(2).setCellRenderer(centerRenderer);
        customer.getColumnModel().getColumn(7).setCellRenderer(centerRenderer);
        customer.getColumnModel().getColumn(5).setCellRenderer(centerRenderer);
        
        JScrollPane scrl=new JScrollPane(customer);
     ...
     private void setDataInTabel(String text)
    {
        ...
        try 
        {
           ResultSet rset=StaticMember.con.createStatement().executeQuery("select * from customer where (customer_name like'%"+text+"%') order by customer_name");
            while(rset.next())
            {
                tableModel.addRow(new Object[]{tableModel.getRowCount()+1," "+rset.getString("customer_name")+" "," "+rset.getString("customer_mob")+"  "," "+rset.getString("customer_address")+" ",rset.getString("customer_email")," "+rset.getString("customer_dl")+" "," "+rset.getString("customer_gst")+" "," "+rset.getString("customer_bank_name")+" "});
    ...
    private void setDataInTabel()
    {
        ...
        try 
        {
           ResultSet rset=StaticMember.con.createStatement().executeQuery("select * from customer order by customer_name");
            while(rset.next())
            {
                tableModel.addRow(new Object[]{tableModel.getRowCount()+1," "+rset.getString("customer_name")+" "," "+rset.getString("customer_mob")+"  "," "+rset.getString("customer_address")+" ",rset.getString("customer_email")," "+rset.getString("customer_dl")+" "," "+rset.getString("customer_gst")+" "," "+rset.getString("customer_bank_name")+" "});
